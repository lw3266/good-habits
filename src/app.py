...
def main_page():
    ...
    # st.markdown(f"<div class='stat-box'>🔄 Login Count: {st.session_state.get('login_count', 0)}</div>", unsafe_allow_html=True)
    ...
    if tabs:
        ...
        st.info("No tab data available. The feature for obtaining real time tab information is not fully functional. We will assume the following configuration.")
    
    if st.button("Get ChatGPT Analysis of Tab Stats"):
        tab_context = get_tab_stat_context(st.session_state["username"])
        print(f"tab usage: {tab_context}")
        query = f"Based on my current tab usage stats:\n{tab_context}\nPlease provide some insights or suggestions."
        response = chat_with_gpt(query, st.session_state["username"])
        st.session_state['chat_response'] = response
        
        # Inject JS to dispatch a custom event for the extension.
        escaped_response = response.replace('"', '\\"').replace('\n', '\\n')
        print(f"escaped_response is {escaped_response}")
        st.markdown(f"""
        <script>
        window.dispatchEvent(new CustomEvent('chatGptNotification', {{detail: "{escaped_response}"}}));
        </script>
